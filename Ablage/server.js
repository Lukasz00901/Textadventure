...
// Chest tracking variable
let chestInteractionAllowed = 0;
...
app.get('/event', (req, res) => {
  ...
  let event;
  if (randomNum < 10 && chestInteractionAllowed === 0) {
  ...
  if (event.type === 'chest') {
    chestInteractionAllowed = 1; // Set chest interaction allowed to 1
    res.json({
  ...
  if (event.type === 'monster') {
    ...
    if (player.hp <= 0) {
      ...
      player.gold = Math.round((player.gold + goldLoot) * 100) / 100;
...
app.post('/open-chest', (req, res) => {
  if (chestInteractionAllowed === 0) {
    res.json({
      message: 'You cannot interact with the chest right now.',
      player
    });
    return;
  }
  ...
  if (decision && decision.toLowerCase() === 'yes') {
    ...
    if (Math.random() < 0.1) {
      ...
      if (player.hp <= 0) {
        player.hp = 50; // Reset HP
        player.roomCounter = 0; // Reset room counter
        chestInteractionAllowed = 0; // Reset chest interaction allowed
        res.json({
          message: `You were killed by the ${monsterName}. Your HP has been restored to 50 and you are back at the start.`,
          player
        });
...
app.post('/difficulty', (req, res) => {
  ...
  if (level && level > 0) {
    difficultyMultiplier = Math.round((1 + (level * 0.2)) * 100) / 100;
